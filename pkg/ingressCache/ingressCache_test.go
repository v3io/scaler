...
type mockFunction func() *mock.MockIngressHostsTree
...
func (suite *IngressCacheTest) TestGet() {
	...
	for _, testCase := range []struct {
		...
		{
			name:           "Gettwo functionName",
			args:           testIngressCacheArgs{testHost, testPath, ""},
			expectedResult: []string{testFunctionName1, testFunctionName2},
			testMocks: func() *mock.MockIngressHostsTree {
				m := &mock.MockIngressHostsTree{}
				m.On("GetPath", testPath).Return([]string{testFunctionName1, testFunctionName2}, nil)
				return m
			},
		}, {
			name:           "Getsingle functionName",
			args:           testIngressCacheArgs{testHost, testPath, ""},
			expectedResult: []string{testFunctionName1},
			testMocks: func() *mock.MockIngressHostsTree {
				m := &mock.MockIngressHostsTree{}
				m.On("GetPath", testPath).Return([]string{testFunctionName1}, nil)
				return m
			},
		}, {
			name:           "Getwith not existing host",
...
func (suite *IngressCacheTest) TestSet() {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			testMocks: func() *mock.MockIngressHostsTree {
				m := &mock.MockIngressHostsTree{}
				m.On("GetPath", testPath).Return([]string{testFunctionName1}, nil).Once()
				m.On("SetPath", testPath, testFunctionName2).Return(nil).Once()
				return m
			},
		}, {
			...
			testMocks: func() *mock.MockIngressHostsTree {
				m := &mock.MockIngressHostsTree{}
				m.On("SetPath", testPath, testFunctionName1).Return(nil).Once()
				return m
			},
		},
	} {
...
func (suite *IngressCacheTest) TestDelete() {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			testMocks: func() *mock.MockIngressHostsTree {
				m := &mock.MockIngressHostsTree{}
				m.On("DeletePath", testPath, testFunctionName2).Return(nil).Once()
				return m
			},
		},
	} {
