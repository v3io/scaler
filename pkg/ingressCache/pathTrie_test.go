...
func (suite *pathTreeTest) TestPathTreeSet() {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			expectedResult: map[string][]string{},
			shouldFail:     true,
			errorMessage:   emptyFunctionNameError,
		}, {
			name: "empty path",
			args: []pathTreeFunctionArgs{
				{
...
func (suite *pathTreeTest) TestPathTreeGet() {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			arg:          "",
			shouldFail:   true,
			errorMessage: walkPathResultError,
		}, {
...
func (suite *pathTreeTest) TestPathTreeDelete() {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			shouldFail:   true,
			errorMessage: functionNotExistsError,
		}, {
...
func (suite *pathTreeTest) pathTreeToMap(t *pathTree) (map[string][]string, error) {
	resultMap := make(map[string][]string)
	err := t.Walk(func(key string, value interface{}) error {
